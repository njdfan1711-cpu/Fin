...
from news_scan import classify_headline_sentiment
...
# "company_news_caution" -- owned by fetch_final_news_caution below, run
# HERE in compose_alerts.py rather than news_scan.py. Deliberately a
# separate channel from NEWS_CAUTION_CATEGORY even though both are
# bearish-news flags: news_scan.py matches against 4 broad general-market
# feeds every 30min, which can miss a company-specific story (a single
# name mentioned once in a niche article won't always make a "general
# market news" feed). This is a narrower, later, per-symbol check
# against Finnhub's company-news endpoint for just the tickers about to
# be pushed -- a final gut-check immediately before send, not a
# replacement for the broader scan.
COMPANY_NEWS_CAUTION_CATEGORY = "company_news_caution"
...
CAUTION_STYLE_CATEGORIES = [
    ...
    (COMPANY_NEWS_CAUTION_CATEGORY, "News caution"),
]
...
def score_ticker(categories: dict) -> tuple:
    ...
    This is the RAW score -- no caution penalty applied. Used ONLY for
    the MIN_QUALIFYING_STRENGTH gate in main(), so a caution flag can
    never silently disqualify a ticker that otherwise earned its spot.
    Everywhere else (ranking, tier display, push title, outcome
    logging) uses rank_score_ticker below instead."""
...
# Per caution-style category present, subtracted from strength for
# ranking/tier purposes (see rank_score_ticker). Calibrated against
# typical qualifying strengths (top picks usually run ~1.9-2.7 total,
# the qualifying floor is MIN_QUALIFYING_STRENGTH=1.0): one flag is
# enough to drop a pick from the top of the list to near the bottom,
# and to flip a borderline 2-category STRONG down to MODERATE, without
# needing multiple flags to matter. Deliberately NOT applied to
# score_ticker/the qualifying gate -- this demotes rank and confidence
# tier, it does not disqualify (that was the explicit call: heavy
# penalty, not a hard exclude).
CAUTION_RANK_PENALTY = 1.5


def rank_score_ticker(categories: dict) -> tuple:
    """Like score_ticker, but for everything AFTER qualification: sort
    order, the displayed tier/strength, the push title's strong-count,
    and the outcome-history log. Applies CAUTION_RANK_PENALTY once per
    caution-style category present (news_caution, earnings_quality,
    caution) -- so a ticker with a bearish-news flag AND an earnings-
    quality flag gets hit twice, compounding rather than capping at one
    flag's worth of doubt."""
    count, strength = score_ticker(categories)
    caution_flags = sum(1 for k in categories if k in CAUTION_STYLE_CATEGORY_KEYS)
    return (count, strength - (CAUTION_RANK_PENALTY * caution_flags))
...
FINAL_NEWS_CHECK_POOL_SIZE = TOP_N_ALERTS + 15  # bigger than TOP_N_ALERTS
                                    # so a flagged ticker's spot can be
                                    # backfilled by the next-best candidate
                                    # instead of just shrinking the push
FINAL_NEWS_CHECK_LOOKBACK_DAYS = 2  # Finnhub company-news date range to
                                    # scan -- wider than news_scan.py's
                                    # 40min lookback since this runs once
                                    # per push (not every cycle) and is
                                    # meant to catch anything the broader
                                    # scan missed, not just this instant


def fetch_final_news_caution(symbol: str) -> dict | None:
    """
    Per-symbol Finnhub company-news check -- a final gut-check for
    tickers about to be pushed, run in addition to (not instead of)
    news_scan.py's broader 4-feed scan. See COMPANY_NEWS_CAUTION_CATEGORY
    above for why this exists as its own channel.

    Returns {"detail": ..., "strength": ...} for the FIRST bearish
    headline found (company-news is returned newest-first by Finnhub),
    or None if nothing bearish turned up / the request failed. A failed
    request returns None rather than raising -- this is a bonus check,
    not something that should be able to take down the whole push if
    Finnhub has a bad moment.
    """
    if not FINNHUB_API_KEY:
        return None
    today = datetime.now(timezone.utc).date()
    from_date = today - timedelta(days=FINAL_NEWS_CHECK_LOOKBACK_DAYS)
    url = (f"https://finnhub.io/api/v1/company-news?symbol={symbol}"
           f"&from={from_date}&to={today}&token={FINNHUB_API_KEY}")
    try:
        ...
        print(f"  [final news check] {symbol}: request failed -- {e}", file=sys.stderr)
        return None
    ...
    for a in articles:
        ...
        if not headline:
            continue
        if classify_headline_sentiment(headline) == "bearish":
            source = a.get("source", "Finnhub")
            return {"detail": f"[{source}] {headline[:130]}", "strength": 0.7}
    return None
...
FETCH_TIMEOUT_SECONDS = 30   # per-request timeout for the shortlist price/ATR
                              # download -- same rationale as filters.py's
                              # BATCH_TIMEOUT_SECONDS: without it, a request
                              # hitting Yahoo mid-rate-limit hangs instead of
                              # failing fast
...
def format_ticker_line(rank: int, symbol: str, name: str, categories: dict,
    ...
    count, strength = rank_score_ticker(categories)
...
def main():
    ...
    ranked = sorted(
        qualifying.items(),
        key=lambda kv: (*rank_score_ticker(kv[1]), tiebreak_key(kv[0])),
        reverse=True,
    )

    # Final per-symbol news gut-check, on a pool somewhat larger than
    # TOP_N_ALERTS -- catches a company-specific story news_scan.py's
    # broader feeds missed, on exactly the tickers that matter (about to
    # be pushed), before send. Deliberately checked on a pool BIGGER than
    # TOP_N_ALERTS: if this flags something in the top 20, a ticker
    # ranked just below still needs a chance to take its place, so the
    # whole pool is re-sorted and re-ranked below rather than checking
    # only the top 20 and leaving a demoted pick's old slot empty.
    check_pool = ranked[:FINAL_NEWS_CHECK_POOL_SIZE]
    flagged = 0
    for sym, cats in check_pool:
        if COMPANY_NEWS_CAUTION_CATEGORY in cats:
            continue  # already flagged this run, e.g. re-run/retry
        caution = fetch_final_news_caution(sym)
        if caution:
            cats[COMPANY_NEWS_CAUTION_CATEGORY] = caution
            flagged += 1
    print(f"Final news check: {len(check_pool)} candidate(s) checked, "
          f"{flagged} flagged on company-specific bearish news.", file=sys.stderr)

    # Re-sort now that the final check may have changed some candidates'
    # rank_score_ticker strength -- cats were mutated in place above, so
    # this picks up any new caution penalties. Ranks are assigned AFTER
    # this, not before, so the displayed #N always matches final order.
    if flagged:
        ranked = sorted(
            ranked,
            key=lambda kv: (*rank_score_ticker(kv[1]), tiebreak_key(kv[0])),
    ...
    for rank, sym, cats in ranked_with_rank:
        count, strength = rank_score_ticker(cats)
    ...
    strong_count = sum(1 for rank, sym, cats in push_list
                        if conviction_tier(*rank_score_ticker(cats)) == "STRONG")
    ...
    if push_list:
        ...
        record_push([
            {
                "symbol": sym,
                "tier": conviction_tier(*rank_score_ticker(cats)),
                "category_count": rank_score_ticker(cats)[0],
                "strength": rank_score_ticker(cats)[1],
                "categories": {
